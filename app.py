...
import os                              # lets you read environment variables, list files, create folders..
from dotenv import load_dotenv         # reads .env and loads each pair into your environment.
...
from PIL import Image                  # (Pillow) for opening, resizing, cropping, and saving images.
import io                              # Lets you treat bytes in RAM as if they were a file, for reading a downloaded image without writing it to disk.
import requests
import json
import time                            # Sleep between API retries, measure how long an operation takes
import base64
from streamlit_drawable_canvas import st_canvas    # draw masks......
import numpy as np                                 # for fast array math (image pixels, masks,
...
load_dotenv(verbose=True)  # Add verbose=True helps debug if the file is missing or badly formatted.

# Debug: Print environment variable status
api_key = os.getenv("BRIA_API_KEY")                # fetch api key
...
def initialize_session_state():
    # function ensures all the important variables are initialized only once
...
def download_image(url):
    ...
    try:
        response = requests.get(url)          # to dwnld
...
def apply_image_filter(image, filter_type):
    ...
    try:                                                  # Wraps everything in a try block to catch and handle any errors that might happen while applying the filter.
        img = Image.open(io.BytesIO(image)) if isinstance(image, bytes) else Image.open(image)
        
        if filter_type == "Grayscale":       
            return img.convert('L')             # the image loses its color and becomes a black-and-white version.
        elif filter_type == "Sepia":            # sepia: Applies a vintage tone manually pixel by pixel
        ...
        # img.point() applies a function to every pixel in the image.
        # Multiplies each pixel value by 1.5, making bright areas brighter and dark areas darker—increasing contrast.
...
def check_generated_images():
    ...
    if st.session_state.pending_urls:              # if there r pending imgs
        # create 2 lists to trackimg stats
        ready_images = []                          # for URLs where the image is ready.
        still_pending = []                         # for urls which r not ready
        
        for url in st.session_state.pending_urls:  # loop through each url
            try:
                response = requests.head(url)      # sends a head request to img url: quick way to check if an image is ready on the server without downloading the full image
                if response.status_code == 200:  
                    ready_images.append(url)
                # A 200 OK status means the image exists and is ready to be accessed. So we add the URL to ready_images.
                else:
                    still_pending.append(url)
            except Exception as e:
                still_pending.append(url)           #  prevents the app from crashing.
...
def auto_check_images(status_container):
    ...
    while attempt < max_attempts and st.session_state.pending_urls:
        time.sleep(2)              # Wait 2 seconds between checks, helps prevent overloading
        if check_generated_images():
            status_container.success("✨ Image ready!")
            return True
        attempt += 1               # Adds 1 to attempt so the loop eventually stops after 3 tries.
    return False
...
def main():
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            # Store original prompt in session state (that box) when it changes
            if "original_prompt" not in st.session_state:
        ...
        with col2:
            ...
            enhance_img = st.checkbox("Enhance image quality", value=False)
                    ...
                    if result:
                        ...
                        if isinstance(result, dict):
                            if "result_url" in result:
                                ...
                                st.image(st.session_state.edited_image, caption="Generated Image", width=300)
                            elif "result_urls" in result:
                                st.session_state.generated_images = result["result_urls"][:num_images]
                                ...
                                for item in result["result"]:
                                    ...
                                    if len(urls) >= num_images:
                                        break
                                st.session_state.generated_images = urls[:num_images]
                                st.success(f"✨ {len(st.session_state.generated_images)} image(s) generated successfully!")

                            """ from 248: First, it prints the raw response for debugging. 
                                    Then, it checks if result is a dictionary and looks for keys like "result_url", 
                                    "result_urls", or "result" (which might be a list of dictionaries or URLs). 
                                    Once it finds a valid image URL, it saves it to st.session_state.edited_image"""
                        else:
                    ...
                    # Display all generated images
                        if st.session_state.get("generated_images"):
                            ...
                            for i, img_url in enumerate(st.session_state.generated_images):
                                with cols[i]:
                                    st.image(img_url, caption=f"Image {i+1}", width=300)
                            ...
                            with st.spinner("Generating lifestyle shot..."):
                                try:
                                    ...
                                    if "422" in str(e):
                                        ...
                                        """When the user selects "Text Prompt" and clicks the "Generate Lifestyle Shot" button, 
                                        it collects all user-defined options like placement type, padding, size, and prompt description. 
                                        It then calls the lifestyle_shot_by_text() function with these inputs. 
                                        If the generation is synchronous (sync_mode), the app immediately checks the response and 
                                        displays the generated image from available URLs. If it's asynchronous, 
                                        it stores the returned URLs in pending_urls and displays a message that generation has started. 
                                        It then either automatically checks if the images are ready (via auto_check_images) or 
                                        gives the user a "Check for Generated Images" button. Throughout the process, exceptions 
                                        are caught and displayed nicely, including specific warnings for content moderation failures (like a 422 error)."""
                    else:
                        ref_image = st.file_uploader("Upload Reference Image", type=["png", "jpg", "jpeg"], key="ref_upload")        # else shows a file uploader to let them upload a reference image 
