...
def generate_hd_image(             # these paras r in bria.ai
    prompt: str,
    api_key: str,
    ...
    if not prompt:
        raise ValueError("Prompt is required for image generation")           # If the prompt is empty, fnctn stops and gives an error.
    
    # Building request data with only provided parameters
    data = {
        "prompt": prompt,
        "num_results": max(1, min(num_results, 4)),    # ensures between 1 and 4 images (the API limit).
        "sync": sync,                                  # wait for the images to be generated and returned immediately.
        "negative_prompt": negative_prompt             # things you don’t want in the image(blurr)
    }
    
    # Add optional parameters to the request above, only if you provided them.
    if aspect_ratio:
