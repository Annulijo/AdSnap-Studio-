...
def generate_ad_set(                        # function is designed to generate various product ad images
    api_key: str,
    image: Optional[bytes] = None,
    prompt: Optional[str] = None,
    config: Dict[str, Any] = None           # dictionary of options for what to generate(packshot...)
) -> Dict[str, Any]:
    """
    Generate a set of product ads based on configuration.
    """
    if not config:                          # If no configuration is provided, initialize it as an empty dictionary.
        config = {}
    ...
    if prompt and not image:
        ...
        image = hd_response.get("result_url")          # Store the response in the results dictionary. Then extract and assign the resulting image URL to image for further processing.
